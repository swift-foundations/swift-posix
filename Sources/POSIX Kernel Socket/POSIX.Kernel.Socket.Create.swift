...
extension POSIX.Kernel.Socket {
    /// Create — L3-policy typealias to the L2-canonical iso-9945 form.
    ///
    /// Per [PLAT-ARCH-005] revised / [PLAT-ARCH-008e] (Wave 3.5 POSIX
    /// wrapping): POSIX-shared socket vocabulary is canonical at L2
    /// (`ISO_9945.Kernel.Socket.Create`); swift-posix contributes the
    /// L3-policy typealias so the three-tier chain
    /// `Kernel.Socket.Create → POSIX.Kernel.Socket.Create →
    /// ISO_9945.Kernel.Socket.Create` composes one tier at a time.
    ...
    public typealias Create = ISO_9945.Kernel.Socket.Create
}
