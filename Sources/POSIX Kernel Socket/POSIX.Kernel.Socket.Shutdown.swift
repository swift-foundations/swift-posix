...
extension POSIX.Kernel.Socket {
    /// Shutdown — L3-policy typealias to the L2-canonical iso-9945 form.
    ///
    /// Per [PLAT-ARCH-005] revised / [PLAT-ARCH-008e] (Wave 3.5 POSIX
    /// wrapping): POSIX-shared socket vocabulary is canonical at L2
    /// (`ISO_9945.Kernel.Socket.Shutdown`); swift-posix contributes the
    /// L3-policy typealias so the three-tier chain
    /// `Kernel.Socket.Shutdown → POSIX.Kernel.Socket.Shutdown →
    /// ISO_9945.Kernel.Socket.Shutdown` composes one tier at a time.
    ...
    public typealias Shutdown = ISO_9945.Kernel.Socket.Shutdown
}
