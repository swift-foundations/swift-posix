...
extension POSIX.Kernel.Socket {
    /// Error — L3-policy typealias to the L2-canonical iso-9945 form.
    ///
    /// Per [PLAT-ARCH-005] revised / [PLAT-ARCH-008e] (Wave 3.5 POSIX
    /// wrapping): POSIX-shared socket vocabulary is canonical at L2
    /// (`ISO_9945.Kernel.Socket.Error`); swift-posix contributes the
    /// L3-policy typealias so the three-tier chain
    /// `Kernel.Socket.Error → POSIX.Kernel.Socket.Error →
    /// ISO_9945.Kernel.Socket.Error` composes one tier at a time.
    ...
    public typealias Error = ISO_9945.Kernel.Socket.Error
}
