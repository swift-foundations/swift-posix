...
extension POSIX.Kernel.Socket {
    /// Bind — L3-policy typealias to the L2-canonical iso-9945 form.
    ///
    /// Per [PLAT-ARCH-005] revised / [PLAT-ARCH-008e] (Wave 3.5 POSIX
    /// wrapping): POSIX-shared socket vocabulary is canonical at L2
    /// (`ISO_9945.Kernel.Socket.Bind`); swift-posix contributes the
    /// L3-policy typealias so the three-tier chain
    /// `Kernel.Socket.Bind → POSIX.Kernel.Socket.Bind →
    /// ISO_9945.Kernel.Socket.Bind` composes one tier at a time.
    ...
    public typealias Bind = ISO_9945.Kernel.Socket.Bind
}
